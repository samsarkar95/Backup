...
export class ListEmpComponent implements OnInit {
  ...
  constructor(public empService: EmployeeService, private router: Router ,
     private orderPipe: OrderPipe, private dialogService: DialogService) {
   ...
   ngOnInit(): void {
    ...
    setTimeout(()=> this.alertswitch3=false,4000);
  }
getAllEmployees(){
...
deleteEmployee(id : number){
  ...
  // setTimeout(() => this.alertswitch2, 3000);
  this.empService.deleteEmployees(id)
  .subscribe(data => {
    
    location.href="/list-emp"})}
  
  this.dialogService.result;
